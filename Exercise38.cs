...
namespace Lessons
{
    public class Exercise38 : Exercise
    {
        ...
        public override void Body()
        {
            ...
            array[0] = min = max = InputNumbers.GetNumberFromConsole<float>("Введено некоректное число.");

            StringBuilder answerToConsole = new StringBuilder("Разница между минимальным и максимальным значениями массива [ ");
            for (int i = 1; i < array.Length; i++)
            {
                Console.Write($"Введите {i} элемент: ");
                array[i] = InputNumbers.GetNumberFromConsole<float>("Введено некоректное число.");
